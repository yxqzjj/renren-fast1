 ...
 * @Version: V1.0.0
 **/
public class JiaTianScBlockServiceImpl extends BlockService implements OperationKeyService {
    ...
    public void operationKey(String blockName) throws InterruptedException {
        ...
        if (isCanSendMsg(scBlock)) {
            ...
            if (StringUtils.isNotEmpty(key)) {
                ...
                if (resultSend) {
                    int resultUpdateDb = updateCommand(MsgConstant.MSG_COMMAND_TYPE_CYCLE_ORDER, WcsScblockDaoImpl.getInstance(), blockName);
    ...
    protected MsgDTO offCar(ScMsgService scMsgService, String mcKey, Integer workPlanType, String blockName, String row, String line, String tier, String withWorkBlockName, String location) {
        MsgDTO msgDTO = null;
        if (isSameArea(row, line, tier, location)) {
            // 空车下车
