package io.renren.wap.command.impl.jiatian;
 ...
 * @Version: V1.0.0
 **/
public class JiaTianScCycleCommandImplAbstract extends AbstractMachineCycleCommand implements ScCommandInterface {
    public WcsScblockEntity scBlock;
    public WcsWorkplanEntity workPlan;

    public JiaTianScCycleCommandImplAbstract(MsgCycleOrderFinishReportDTO msgCycleOrderFinishReportDTO) {
    ...
    public void unload() throws InterruptedException {
        //  卸货  任务结束
        WcsScblockDaoImpl.getInstance().updateUnloadJiaTianFinishByPrimaryKey(blockName, false, new Date(), row, line, tier);
        MsgCycleOrderFinishReportAckService.replay05AndUpdateBlock(msgCycleOrderFinishReportAckConditionDTO);
        //  任务完成操作
        try {
    ...
    private void goMc() throws InterruptedException {
        ...
        if (withWorkBlockName.equals(toStation)) {
            WcsMcblockEntity mcBlock = WcsMcblockDaoImpl.getInstance().getInstance().selectByPrimaryKey(withWorkBlockName);
